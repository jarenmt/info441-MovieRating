...
sudo docker pull tilleyjaren/server
sudo docker pull tilleyjaren/tilleyjarensql
...
sudo docker run -d --name mysqlServer  --network 441network -p 3306:3306 -e MYSQL_ROOT_PASSWORD=$MYSQL_ROOT_PASSWORD -e MYSQL_DATABASE=$DB_NAME tilleyjaren/tilleyjarensql
export DSN=root:$MYSQL_ROOT_PASSWORD@tcp\(mysqlServer:3306\)/$DB_NAME
sudo docker run -d -p 443:443 -e ADDR=$ADDR --name gateway -v /etc/letsencrypt:/etc/letsencrypt:ro -e TLSCERT=$TLSCERT -e TLSKEY=$TLSKEY -e DSN=$DSN -e SESSIONKEY=$SESSIONKEY --network 441network tilleyjaren/server
